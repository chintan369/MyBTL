...
import com.agraeta.user.btl.adapters.RegisteredDealerListAdapter;
...
public class RegisteredDealerListActivity extends AppCompatActivity implements Callback<RegisteredDealerData> {
    ...
    RegisteredDealerListAdapter adapter;
    ...
    private void showFilterDialog() {
        ...
        edt_fromDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog datePickerDialog = new DatePickerDialog(RegisteredDealerListActivity.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
        ...
        btn_submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!fromDate.isEmpty() && !toDate.isEmpty()) {
                    ...
                    page = 1;
                    totalPage = 1;
                    /*careerDataCall.cancel();
                    careerDetailList.clear();
                    careerListAdapter.notifyDataSetChanged();
                    careerDataCall = adminAPI.careerDataCall(fromDate, toDate, page);
                    careerDataCall.enqueue(CareerListActivity.this);*/
                } else {
    ...
    private void fetchIDs() {
        ...
        adapter=new RegisteredDealerListAdapter(dealerList,this);
        list_enquiries.setAdapter(adapter);

        txt_enquiryName.setText(enquiryName);

        progressDialog.show();

        Call<RegisteredDealerData> registeredDealerDataCall=adminAPI.getRegisteredDealer();
        registeredDealerDataCall.enqueue(this);

    }

    @Override
    public void onResponse(Call<RegisteredDealerData> call, Response<RegisteredDealerData> response) {
        progressDialog.dismiss();
        RegisteredDealerData data=response.body();

        dealerList.clear();

        if(data!=null && data.isStatus()){
            dealerList.addAll(data.getData());
    ...
    @Override
    public void onFailure(Call<RegisteredDealerData> call, Throwable t) {
        progressDialog.dismiss();
